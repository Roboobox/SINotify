...
def run_check():
    ...
    if GetPreviousId.previous_id != new_id:
        ...
        link_new_changes = os.environ['link_new_changes'] + str(new_record_amount)
        ...
        for key, value in fullChanges.items():
            ...
            link_c = os.environ['link_c_sec'] + key + "&Msg=" + value
            try:
                send_request = urllib.request.urlopen(link_c, timeout=180)
                send_request.close()
            except urllib.error.HTTPError as e:
                print("Connection error (Code " + str(e.code) + ") (Reason - " + str(e.reason) + ")")
